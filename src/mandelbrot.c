...
/*   Updated: 2017/06/28 23:22:09 by rlecart          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ppm(t_menu *men)
{
	...
	i = 0;
	if (men->m.x < men->m.image_x
			&& men->m.y < men->m.image_y
			&& men->m.x >= 0 && men->m.y > 0)
	{
...
void	mandelbrot_loop(t_menu *men, int tmp[5])
{
	...
	men->m.z_i = 0;
	men->m.i = 0;
	while ((men->m.z_r * men->m.z_r + men->m.z_i * men->m.z_i) < 4
			&& men->m.i++ < men->m.imax)
	{
		men->m.tmp = men->m.z_r;
		men->m.z_r = men->m.z_r * men->m.z_r - men->m.z_i * men->m.z_i
			+ men->m.c_r;
...
{
	if (!men->m.win)
		men->m.win = mlx_new_window(men->mlx,
				men->m.image_x, men->m.image_y, "Mandelbrot");
	...
	while (men->m.x < men->m.image_x)
	{
		...
		{
			mandelbrot_loop(men, (int[5]){0, 0, 0, 0, 0});
			ppm(men);
			men->m.y++;
		}
		men->m.x++;
